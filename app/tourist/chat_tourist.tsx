...
export default function ChatTourist() {
  ...
  const getAvatarUri = () => {
    if (!guideAvatar) return null;
    return guideAvatar.startsWith("http") ? guideAvatar : `${API_URL}${guideAvatar}`;
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        e.message ||
          "Failed to send message. Chat is only available after the guide accepts the booking."
      );
    } finally {
      setSending(false);
    }
  ...
  const renderItem = ({ item }: { item: ChatMessage }) => {
    const isMine = item.sender === "tourist";
    return (
      <View
        style={[
          styles.messageRow,
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            ...
            router.push({
              ...
              params: {
                guideId: counterpartId,
                guideName,
                guideImage: guideAvatar ?? undefined,
              },
            });
          }}
          activeOpacity={0.7}

...
import {
  fetchTouristPaymentDetails,
  patchTouristPaymentDetails,
  type PayoutMethod,
  type TouristPaymentDetailsDto,
} from "../../api/touristPayment";
import ScreenHeader from "../../components/screen-header";
import { SkeletonProfileScreen } from "@/components/Skeleton";

function isPaymentConfigured(d: TouristPaymentDetailsDto | null): boolean {
...
export default function TouristPaymentScreen() {
  ...
  const [baseline, setBaseline] = useState<TouristPaymentDetailsDto | null>(null);
  ...
  const applyDto = useCallback((d: TouristPaymentDetailsDto) => {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const dto = await fetchTouristPaymentDetails();
      applyDto(dto);
      setBaseline(dto);
      setEditing(!isPaymentConfigured(dto));
    } catch (e: unknown) {
      ...
      Alert.alert("Could not load", err.message || "Failed to load payment details.");
    } finally {
      setLoading(false);
    }
  }, [applyDto, router]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const dto =
        method === "esewa"
          ? await patchTouristPaymentDetails({
              payoutMethod: "esewa",
      ...
      Alert.alert("Saved", "Your payment details were updated.");
    } catch (e: unknown) {
      const err = e as Error & { status?: number };
      if (err.status === 401 || err.message === "Not logged in") {
        Alert.alert("Session expired", "Please sign in again.", [
      ...
      Alert.alert("Could not save", err.message || "Failed to save payment details.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const showSummary = isPaymentConfigured(baseline) && !editing;
        ...
        {showSummary && baseline ? (
          <>
            <View style={styles.summaryCard}>
              <View style={styles.summaryTop}>
                <Text style={styles.summaryTitle}>Your refund details</Text>
                <View
                  style={[
                    styles.methodPill,
              ...
              {lastSavedLabel ? (
                ...
                <Text style={styles.summaryOnFile}>
                  On file for refunds — not shown on your public profile.
                </Text>
              )}
            </View>
            ...
            <View style={styles.summaryFootnote}>
              ...
              <Text style={styles.summaryFootnoteText}>
                Booking payments still go through eSewa at checkout. These details are used if a paid
                booking is refunded.
              </Text>
            </View>
          </>
          ...
          <>
            <View style={styles.heroCard}>
              ...
              <Text style={styles.heroTitle}>Where refunds go</Text>
              <Text style={styles.heroBody}>
                If a paid booking is cancelled, we use these details to send your refund. Paying for
                tours still uses eSewa at checkout — this is separate from that.
              </Text>
            </View>
            ...
            <View style={styles.notice}>
              ...
              <Text style={styles.noticeText}>
                Stored privately on your account. Not visible to guides on your profile.
              </Text>
            </View>
            ...
            {method === "esewa" ? (
              <View style={styles.form}>
                ...
                <Text style={styles.hint}>
                  Use the ID or mobile number linked to the eSewa wallet where you receive refunds.
                </Text>
              </View>
            ) : (
              <View style={styles.form}>
                ...
                />
                <Text style={styles.hint}>
                  Double-check the account number — wrong digits delay refunds.
                </Text>
              </View>
            )}

            {isPaymentConfigured(baseline) && editing ? (
              <TouchableOpacity style={styles.cancelButton} onPress={handleCancelEdit} activeOpacity={0.8}>
                <Text style={styles.cancelButtonText}>Cancel</Text>
              </TouchableOpacity>
            ) : null}

            {lastSavedLabel && editing && isPaymentConfigured(baseline) ? (
